...
interface IMentalPoker {
    ...
    function reveal(bytes[] calldata revealTokens, bytes calldata masked) external pure returns (bytes memory);
    ...
    // for test
    function test(bytes calldata param1, bytes[] calldata param2) external view returns (bytes memory);
}
